import 'package:flutter/material.dart';
import 'package:google_fonts/google_fonts.dart';

TextStyle navBarItemsStyle =
    GoogleFonts.montserrat(fontSize: 15, color: Colors.black);
TextStyle hintStyle = GoogleFonts.poppins(
  color: const Color(0XFF242424),
  fontSize: 13,
  fontWeight: FontWeight.w400,
);
TextStyle heading1Style = GoogleFonts.poppins(
  fontSize: 24,
  fontWeight: FontWeight.w500,
  color: const Color(0XFF242424),
);
TextStyle heading2Style = GoogleFonts.poppins(
  fontSize: 14,
  fontWeight: FontWeight.w400,
  color: const Color(0XFF242424),
);
TextStyle btnStyle = GoogleFonts.poppins(
  fontSize: 18,
  color: Colors.white,
  fontWeight: FontWeight.w600,
);
TextStyle outlineBtnStyle = GoogleFonts.poppins(
  fontSize: 18,
  color: Colors.white,
  fontWeight: FontWeight.w600,
);
TextStyle iconTextStyle = const TextStyle(
  fontSize: 10,
  color: Color(0XFF78aecd),
);
TextStyle testTextStyle = GoogleFonts.poppins(
  fontSize: 18,
  color: const Color(0XFF242424),
  fontWeight: FontWeight.w500,
);
TextStyle testCartTextStyle = const TextStyle(
  fontSize: 12,
  color: Color(0XFF919191),
  fontWeight: FontWeight.w700,
);
TextStyle cartCheckoutTextStyle = const TextStyle(
  fontSize: 15,
  color: Color(0XFFd5d5d5),
  fontWeight: FontWeight.w400,
);
TextStyle orderHistoryTextStyle = GoogleFonts.poppins(
  fontSize: 14,
  color: Colors.black,
  fontWeight: FontWeight.w500,
);
TextStyle profileTextStyle = const TextStyle(
    color: Colors.white, fontWeight: FontWeight.w700, fontSize: 17);
